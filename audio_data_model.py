...
class AudioDataModel: #initialzing class
    ...
    def estimate_rt60(self, audio_signal):
        ...
        # Calculate the time it takes for the energy to decay by 60 dB
        # First, find the time where the signal starts (non-zero part)
        start_idx = np.argmax(energy_db > -40)  # Ignore initial noise, below -40 dB
        if start_idx == 0:
            return 0  # Handle case where there's no discernible signal
        ...
        # Estimate RT60 as the time it takes for the signal to decay by 60 dB
