...
class AudioDataView:
    ...
    def update_summary_box(self, file_name, duration, max_amplitude, rt60_values):
        ...
        self.max_amplitude_label.config(text=f"Max Amplitude (Hz): {max_amplitude:.2f}")
    ...
    def update_rt60_diff(self, diff_low, diff_mid, diff_high):
        ...
        self.diff_low_label = tk.Label(self.summary_box, text=f"Low: {diff_low:.2f} seconds")
        self.diff_low_label.grid(row=5, column=0, sticky="w")

        self.diff_mid_label = tk.Label(self.summary_box, text=f"Mid: {diff_mid:.2f} seconds")
        self.diff_mid_label.grid(row=6, column=0, sticky="w")

        self.diff_high_label = tk.Label(self.summary_box, text=f"High: {diff_high:.2f} seconds")
        self.diff_high_label.grid(row=7, column=0, sticky="w")
